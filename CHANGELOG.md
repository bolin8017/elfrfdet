...
- Bumped `maldet[mlflow]` pin to `>=2.2.1,<3.0` and `[compat] min_maldet = "2.2"` to pick up the MLflow data-model redesign:
  ...
  - MLflow Models flavor — trained models now ship with `MLmodel` YAML, `python_env.yaml`, and an inferred signature; downstream evaluate/predict containers load via `mlflow.sklearn.load_model`
  - Dataset lineage via `mlflow.log_input` per stage
  - System metrics (`system/cpu_*`, `system/system_memory_*`) auto-logged via `psutil` pulled in by `maldet[mlflow]>=2.2.1`
